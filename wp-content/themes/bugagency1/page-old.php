<?php
/**
 * The template for displaying all pages
 *
 * This is the template that displays all pages by default.
 * Please note that this is the WordPress construct of pages
 * and that other 'pages' on your WordPress site may use a
 * different template.
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package bugagency
 */

get_header();
?>

    <div class="content_elements">
        <section id="section_intro" class="section-item section-intro">
            <div class="section-background">
                <svg xmlns="http://www.w3.org/2000/svg" width="295.761" height="605.776" viewBox="0 0 295.761 605.776">
                    <g id="Group_1" data-name="Group 1" transform="translate(0 -1657.955)">
                        <path id="Path_203" data-name="Path 203" d="M-35.038-1273.73v179.312L43.721-1045.3l217,135.343v-184.464Z" transform="translate(35.038 2931.685)" fill="#2e3636"/>
                        <path id="Path_202" data-name="Path 202" d="M-35.038-1273.73v179.312L43.721-1045.3l217,135.343v-184.464Z" transform="translate(35.038 3173.685)" fill="#2e3636" opacity="0.37"/>
                    </g>
                </svg>
            </div>
            <div class="section-item-content">
                <div class="section-item-inner">
                    <div class="container">
                        <div class="row">
                            <div class="col-xl-6">
                                <div class="section-intro-hero">
                                    <h1>
                                        <span class="stroked-text">BUG AGENCY</span>  <br>Creative Studio
                                    </h1>
                                    <p>
                                        Bringing Inovation and ideas together <br>
                                        and offering creative solutions
                                    </p>
                                </div>
                            </div>
                            <div class="col-xl-6">
                                <div class="animation_box_wrapper">
                                    <div class="typewritter_animation_box">
                                        <img src="wp-content/uploads/assets/laptop-home.png" alt="">
                                        <div class="typewritter_animation_box_content " >
                                            <h6 id="typedtext"></h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_about" class="section-item section-about">
            <div class="section-item-content "  style="background-image: url('wp-content/uploads/assets/aboutbackground.png')">
                <div class="about-section-inner">
                    <div class="about-title-section">
                        <div class="container">
                            <div class="title-section">
                                <h2 class="text-animation">
                                    <span class="stroked-text right-part">ABOUT US</span><br>
                                    We Are Experts<br>
                                    <span class="right-part">In What We Do</span>
                                </h2>
                            </div>
                        </div>
                    </div>
                    <div class="container">
                        <div class="content-section scroller">
                            <p>
                                We are committed to delivering values to your projects and your needs. Creating authentic website is what we do best. Supporting your business to get recognised and grow. Expand your presence online with our help. Providing expertise and customised solutions. Get your brand recognised.
                            </p>
                            <p>
                                We offer a wide range of services to support your goals and needs.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_services" class="section-item services-section">
            <div class="section-background">
                <svg xmlns="http://www.w3.org/2000/svg" width="295.761" height="605.776" viewBox="0 0 295.761 605.776">
                    <g id="Group_1" data-name="Group 1" transform="translate(0 -1657.955)">
                        <path id="Path_203" data-name="Path 203" d="M-35.038-1273.73v179.312L43.721-1045.3l217,135.343v-184.464Z" transform="translate(35.038 2931.685)" fill="#2e3636"/>
                        <path id="Path_202" data-name="Path 202" d="M-35.038-1273.73v179.312L43.721-1045.3l217,135.343v-184.464Z" transform="translate(35.038 3173.685)" fill="#2e3636" opacity="0.37"/>
                    </g>
                </svg>
            </div>
            <div class="section-item-content">
                <div class="services-section-inner">
                    <div class="container">
                        <div class="services-section-intro">
                            <h6>SERVICES</h6>
                            <h3 class="text-uppercase"><span class="stroked-text">What We</span> Do...</h3>
                            <p>
                                Loyalty at our core. <br>
                                Web and app solutions that help your clients value what you do.
                            </p>
                        </div>
                        <div class="services-section-flipcards-slider-wrapper">
                            <div class="services_slider_arrows">
                                <div class="services_slider_arrow prev_slide_service">PREV</div>
                                <div class="services_slider_arrow next_slider_service">NEXT</div>
                            </div>
                            <div class="services-section-flipcards">
                                <div class="flip-card">
                                    <div class="flip-card-inner">
                                        <div class="flip-card-front">
                                            <h5>WEB DEVELOPMENT</h5>
                                            <div class="icon_box">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="63.05" height="62.919" viewBox="0 0 63.05 62.919">
                                                    <g id="Group_9" data-name="Group 9" transform="translate(-768 -359.727)">
                                                        <path id="Path_207" data-name="Path 207" d="M768,425.394c.2-1.372.341-2.753.6-4.113a30.3,30.3,0,0,1,4.855-11.507.992.992,0,0,0,.128-1.149,5.421,5.421,0,0,1,1.279-6.3,5.535,5.535,0,0,1,9.222,4.981,5.454,5.454,0,0,1-4.653,4.52,8.684,8.684,0,0,1-2.312-.153.578.578,0,0,0-.7.276,26.381,26.381,0,0,0-4.616,17.334c.95,10.429,6.17,18.121,15.489,22.85a26.248,26.248,0,0,0,21.645,1.12c.308-.112.611-.237.92-.346a1.843,1.843,0,1,1,1.328,3.435,32.1,32.1,0,0,1-9.343,2.255,3.4,3.4,0,0,0-.472.1h-3.817c-.4-.051-.8-.107-1.207-.151a31.67,31.67,0,0,1-27.863-25.895c-.207-1.143-.325-2.3-.484-3.454ZM780.456,406.4a1.828,1.828,0,0,0-1.808-1.866,1.87,1.87,0,0,0-1.87,1.82A1.827,1.827,0,0,0,778.6,408.2,1.8,1.8,0,0,0,780.456,406.4Z" transform="translate(0 -36.059)" fill="#56256b"/>
                                                        <path id="Path_208" data-name="Path 208" d="M963.458,393.023c-.155,1.132-.281,2.269-.469,3.4a30.457,30.457,0,0,1-4.994,12.233,1.01,1.01,0,0,0-.118,1.151,5.53,5.53,0,1,1-5.889-3.21,8.039,8.039,0,0,1,2.314.136c.364.056.547.02.748-.285a27.5,27.5,0,0,0-4.194-35.6,27.389,27.389,0,0,0-28.722-5.544,4.759,4.759,0,0,1-1.053.318,1.784,1.784,0,0,1-2.01-1.331,1.806,1.806,0,0,1,1.076-2.162,30.966,30.966,0,0,1,8.172-2.173,30.367,30.367,0,0,1,22.115,5.805A30.852,30.852,0,0,1,963.1,386.544c.141.886.238,1.779.356,2.669ZM951,412.055a1.825,1.825,0,0,0,1.845,1.827,1.87,1.87,0,0,0,1.832-1.859,1.828,1.828,0,0,0-1.863-1.811A1.8,1.8,0,0,0,951,412.055Z" transform="translate(-132.408 0)" fill="#56256b"/>
                                                        <path id="Path_210" data-name="Path 210" d="M879.327,450.442A20.281,20.281,0,1,1,859,470.732,20.333,20.333,0,0,1,879.327,450.442Zm5.531,18.408c-.126-1.3-.21-2.544-.374-3.779a23.7,23.7,0,0,0-2.372-8.2,9.664,9.664,0,0,0-1.5-2.076,1.576,1.576,0,0,0-2.567-.022,7.276,7.276,0,0,0-1.072,1.335,18.406,18.406,0,0,0-2.425,6.708c-.357,1.981-.557,3.99-.833,6.038ZM873.788,472.6c.124,1.28.208,2.523.371,3.755a23.266,23.266,0,0,0,2.371,8.2,10.258,10.258,0,0,0,1.779,2.314,1.324,1.324,0,0,0,2.037,0,6.819,6.819,0,0,0,1.343-1.577c2.32-3.9,2.842-8.251,3.147-12.7Zm22.018-3.749c-.206-5.346-5.528-12.33-10.247-13.419a23.725,23.725,0,0,1,2.13,6.533c.388,2.272.65,4.565.973,6.886Zm-22.7-13.5c-5.935,2.335-9.945,8.242-10.227,13.5h7.233A35.061,35.061,0,0,1,873.108,455.358Zm-2.992,17.237h-7.251c-.165,4.85,5.835,12.735,10.192,13.385A34.925,34.925,0,0,1,870.116,472.595Zm15.422,13.487c5.726-2.24,9.879-7.959,10.232-13.47h-7.245A34.964,34.964,0,0,1,885.539,486.082Z" transform="translate(-79.797 -79.544)" fill="#56256b"/>
                                                    </g>
                                                </svg>
                                            </div>
                                        </div>
                                        <div class="flip-card-back">
                                            <div class="flip-card-back-content">
                                                <p>
                                                    Concepting and visualizing your web from scratch.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="flip-card">
                                    <div class="flip-card-inner">
                                        <div class="flip-card-front">
                                            <h5>APP DEVELOPMENT</h5>
                                            <div class="icon_box">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="60.353" height="68" viewBox="0 0 60.353 68">
                                                    <g id="Group_10" data-name="Group 10" transform="translate(-1509.603 -981.87)">
                                                        <path id="Path_211" data-name="Path 211" d="M1516.643,1049.871a9.2,9.2,0,0,1-3.871-1.62,7.926,7.926,0,0,1-3.158-6.1c-.016-.331-.009-.664-.009-1q0-25.531,0-51.063a7.938,7.938,0,0,1,6.231-8.017,8.894,8.894,0,0,1,1.973-.2q16.766-.018,33.532-.006a7.96,7.96,0,0,1,8.2,8.164c.008,2.192,0,4.383,0,6.607a.73.73,0,0,1-.729.729h-3.854a.729.729,0,0,1-.729-.729h0c0-2.191.007-4.383,0-6.574a2.672,2.672,0,0,0-2.907-2.887q-16.766-.006-33.533,0a2.879,2.879,0,0,0-2.1.778,3.876,3.876,0,0,0-.474.889,7.446,7.446,0,0,0-.289,1.192h0v51.66h0a5.583,5.583,0,0,0,.192,1.1,2.673,2.673,0,0,0,.556.961,2.938,2.938,0,0,0,2.129.792c2.063-.047,22.211,0,33.521,0a2.909,2.909,0,0,0,2.909-2.9c.008-2.257,0-4.515,0-6.773v-.1a.664.664,0,0,1,.664-.665h3.983a.664.664,0,0,1,.665.665c0,2.28,0,4.559,0,6.839a7.939,7.939,0,0,1-6.8,8.145,1.262,1.262,0,0,0-.242.1Z" transform="translate(0 0)" fill="#56256b"/>
                                                        <path id="Path_212" data-name="Path 212" d="M1752.89,1164.263h-2.748a1.332,1.332,0,0,1-1.281-1.693l6.678-23.71a1.331,1.331,0,0,1,1.281-.97h2.753a1.331,1.331,0,0,1,1.281,1.693l-6.683,23.71A1.331,1.331,0,0,1,1752.89,1164.263Z" transform="translate(-207.376 -135.258)" fill="#56256b"/>
                                                        <path id="Path_213" data-name="Path 213" d="M1635.178,1164.987l1.712,2.021a1.331,1.331,0,0,1-.153,1.874l-5.095,4.334a1.331,1.331,0,0,0,0,2.028l5.095,4.336a1.331,1.331,0,0,1,.153,1.874l-1.709,2.018a1.332,1.332,0,0,1-1.878.155l-9.87-8.379a1.332,1.332,0,0,1,0-2.03l9.868-8.383A1.332,1.332,0,0,1,1635.178,1164.987Z" transform="translate(-98.275 -158.342)" fill="#56256b"/>
                                                        <path id="Path_214" data-name="Path 214" d="M1856.434,1167.016l1.712-2.018a1.332,1.332,0,0,1,1.877-.153l9.868,8.379a1.332,1.332,0,0,1,0,2.03l-9.867,8.382a1.332,1.332,0,0,1-1.878-.153l-1.712-2.017a1.332,1.332,0,0,1,.152-1.876l5.095-4.336a1.331,1.331,0,0,0,0-2.027l-5.1-4.335A1.331,1.331,0,0,1,1856.434,1167.016Z" transform="translate(-300.404 -158.352)" fill="#56256b"/>
                                                        <rect id="Rectangle_64" data-name="Rectangle 64" width="10.562" height="5.223" rx="2.611" transform="translate(1529.288 992.546)" fill="#56256b"/>
                                                        <path id="Path_215" data-name="Path 215" d="M1679.926,1378.543a2.649,2.649,0,1,1,2.683-2.934A2.652,2.652,0,0,1,1679.926,1378.543Z" transform="translate(-145.401 -339.294)" fill="#56256b"/>
                                                    </g>
                                                </svg>

                                            </div>
                                        </div>
                                        <div class="flip-card-back">
                                            <div class="flip-card-back-content">
                                                <p>Customized app solutions for your business</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="flip-card">
                                    <div class="flip-card-inner">
                                        <div class="flip-card-front">
                                            <h5>UI/UX DESIGN</h5>
                                            <div class="icon_box">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="50.894" height="62.316" viewBox="0 0 50.894 62.316">
                                                    <g id="Group_17" data-name="Group 17" transform="translate(-2203.245 -2067)">
                                                        <g id="Group_16" data-name="Group 16">
                                                            <g id="Group_11" data-name="Group 11" transform="translate(2210.68 2067)">
                                                                <path id="Path_217" data-name="Path 217" d="M2357.716,1428.454a15.554,15.554,0,0,0,2.953,8.919c3.644,5.063,9.771,7.013,15.228,4.678a13.544,13.544,0,0,0,8.549-11.639c.475-4.823.232-9.721.222-14.585a3.29,3.29,0,0,0-.815-1.823,2.823,2.823,0,0,0-3.253-.74,2.986,2.986,0,0,0-1.882,2.868c-.013,3.963,0,7.926-.01,11.889a9.8,9.8,0,0,1-2.121,6.258,6.669,6.669,0,0,1-10.741.018,9.752,9.752,0,0,1-2.133-6.251c-.015-3.933,0-7.866-.01-11.8a3,3,0,1,0-6,.035c-.01,2.09,0,4.18,0,6.273h.005C2357.709,1424.523,2357.672,1426.491,2357.716,1428.454Z" transform="translate(-2357.694 -1413.043)" fill="#56256b"/>
                                                                <path id="Path_218" data-name="Path 218" d="M2480.749,1443.028a3.049,3.049,0,0,0,3.008-3.341q.007-5.8,0-11.6,0-5.754,0-11.509a5.332,5.332,0,0,0-.071-1.115,2.994,2.994,0,0,0-5.923.725q-.014,11.743,0,23.486A3.053,3.053,0,0,0,2480.749,1443.028Z" transform="translate(-2447.735 -1413.065)" fill="#56256b"/>
                                                            </g>
                                                            <path id="Path_219" data-name="Path 219" d="M2353.022,1619.942a2.977,2.977,0,0,0-4.073,2.865c-.027,3.963,0,7.926-.012,11.889a9.846,9.846,0,0,1-2.11,6.261,6.649,6.649,0,0,1-10.722.027,9.746,9.746,0,0,1-2.133-6.251c-.015-3.963,0-7.926-.013-11.889a2.992,2.992,0,1,0-5.978.022c-.012,4.118-.047,8.239.018,12.357a15.237,15.237,0,0,0,2.986,8.9c4.355,5.983,12.1,7.349,17.827,3.118a14.164,14.164,0,0,0,5.885-10.141,72.5,72.5,0,0,0,.233-7.849h0c0-2.153.013-4.308-.005-6.461A3,3,0,0,0,2353.022,1619.942Z" transform="translate(-124.715 479.616)" fill="#56256b"/>
                                                            <path id="Path_220" data-name="Path 220" d="M2462.286,1635.511a1.237,1.237,0,0,1-.025-1.57c1.986-3.1,3.958-6.2,5.834-9.364a4.236,4.236,0,0,0,.542-2.455,2.629,2.629,0,0,0-2.35-2.335,2.839,2.839,0,0,0-3.055,1.358c-1.473,2.315-2.913,4.648-4.371,6.974-.173.275-.365.54-.593.878-.238-.36-.413-.613-.575-.875-1.438-2.3-2.853-4.611-4.321-6.891a3,3,0,1,0-5,3.285c1.976,3.138,3.916,6.3,5.916,9.424a1.236,1.236,0,0,1-.018,1.568c-2,3.125-3.953,6.276-5.906,9.429a2.994,2.994,0,1,0,5.076,3.151c1.435-2.265,2.845-4.546,4.27-6.816.16-.255.338-.5.581-.858.255.4.423.66.587.923,1.423,2.273,2.836,4.551,4.27,6.813a2.993,2.993,0,1,0,5.044-3.206Q2465.27,1640.206,2462.286,1635.511Z" transform="translate(-214.583 479.619)" fill="#56256b"/>
                                                        </g>
                                                    </g>
                                                </svg>
                                            </div>
                                        </div>
                                        <div class="flip-card-back">
                                            <div class="flip-card-back-content">
                                                <p>Unforgivable user experience in your apps and websites</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="flip-card">
                                    <div class="flip-card-inner">
                                        <div class="flip-card-front">
                                            <h5>BRANDING</h5>
                                            <div class="icon_box">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="63.711" height="63.713" viewBox="0 0 63.711 63.713">
                                                    <g id="Group_15" data-name="Group 15" transform="translate(-1697.996 -1755.111)">
                                                        <path id="Path_221" data-name="Path 221" d="M1721.888,1755.111h1c1.765.733,1.3,2.348,1.391,3.7h7.465c0-.731-.05-1.434.013-2.127.073-.8.685-1.221,1.334-1.573h1c1.745.73,1.314,2.306,1.379,3.733h8.924c1.58,0,2.264.687,2.265,2.274q0,11.6,0,23.2v.782h.733c3.939,0,7.879.029,11.818-.018a2.38,2.38,0,0,1,2.506,1.387v24.016a3.038,3.038,0,0,0-.107.348,9.422,9.422,0,0,1-9.506,7.987q-25.975.008-51.95,0a1.909,1.909,0,0,1-2.148-2.142q0-27.841,0-55.683a1.909,1.909,0,0,1,2.136-2.152q4.23,0,8.461,0h.746c0-.76-.05-1.464.013-2.157.073-.8.685-1.223,1.335-1.576h1c1.765.733,1.3,2.348,1.391,3.7h7.465c0-.731-.051-1.434.013-2.127C1720.627,1755.881,1721.239,1755.463,1721.888,1755.111Zm21,7.511h-7.437c0,.561,0,1.077,0,1.594a1.877,1.877,0,1,1-3.732.024c0-.549,0-1.1,0-1.626h-7.466c0,.571,0,1.106,0,1.641a1.874,1.874,0,1,1-3.732.008c0-.551,0-1.1,0-1.635h-7.467c0,.61.008,1.169,0,1.728a1.867,1.867,0,1,1-3.729-.007c-.009-.574,0-1.148,0-1.729h-7.546v52.414h27.873l-1.632-3.679c-.607,0-1.311.007-2.013,0a9.324,9.324,0,0,1-9.081-8.072,42.713,42.713,0,0,1-.129-4.966,5.583,5.583,0,0,1,5.728-5.747c1.617-.009,3.234,0,4.852,0h.613c0-1.817,0-3.536,0-5.256a1.93,1.93,0,0,1,2.2-2.209h12.7Zm-11.17,26.237v.6q0,9.919,0,19.838a5.593,5.593,0,0,0,5.813,5.79q7.245.01,14.49,0a5.724,5.724,0,0,0,5.951-5.952q0-9.795,0-19.589v-.687Zm-3.758,7.446c-1.946,0-3.831-.028-5.715.012a1.81,1.81,0,0,0-1.72,1.8c-.025.725-.031,1.452,0,2.177a14.438,14.438,0,0,0,.215,3.022c.769,2.951,3.333,4.808,7.22,4.231Z" transform="translate(0)" fill="#56256b"/>
                                                        <path id="Path_222" data-name="Path 222" d="M1884.01,1849.4c-1.2,0-2.4.006-3.6,0a1.87,1.87,0,1,1,.018-3.729q3.571-.009,7.14,0a1.873,1.873,0,1,1-.018,3.73C1886.37,1849.409,1885.19,1849.4,1884.01,1849.4Z" transform="translate(-150.419 -75.504)" fill="#56256b"/>
                                                        <path id="Path_223" data-name="Path 223" d="M1884.007,1894.317c-1.2,0-2.4.007-3.6,0a1.87,1.87,0,1,1,.025-3.729q3.57-.009,7.14,0a1.873,1.873,0,1,1-.025,3.73C1886.367,1894.321,1885.187,1894.317,1884.007,1894.317Z" transform="translate(-150.419 -112.95)" fill="#56256b"/>
                                                    </g>
                                                </svg>
                                            </div>
                                        </div>
                                        <div class="flip-card-back">
                                            <div class="flip-card-back-content">
                                                <p>Branding that will get your company recognition in the digital world</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_projects" class="section-item section-projects">
            <div class="section-item-content">
                <div class="container">
                    <h3 class="text-uppercase"><span class="stroked-text">Latest Projects</span></h3>
                    <div class="section-projects-content">

                        <div class="project-item">
                            <a class="project-item-thumbnail" href="https://workflow.al/" target="_blank">
                                <img src="wp-content/uploads/assets/workflow.jpg" alt="">
                            </a>
                            <div>
                                <h2>Workflow</h2>
                                <h5>Website 3.0</h5>
                                <p>Welcome to Workflow, the client we proudly served through our agency. Here, we present the remarkable work we've accomplished in the realm of digital websites.</p>
                                <p>By leveraging the power of Web 3.0 technologies, such as AI-driven elements, immersive user interfaces, and seamless workflows, we've pushed the boundaries of website design. Our approach ensures that each website we've built for Workflow not only boasts a visually stunning aesthetic, but also delivers exceptional user experiences and drives tangible results.</p>
                            </div>
                        </div>

                        <div class="project-item reverse">
                            <a class="project-item-thumbnail" href="https://bringposta-ks.com/" target="_blank">
                                <img src="wp-content/uploads/assets/project1.jpg" alt="">
                            </a>
                            <div>
                                <h2>Bring Posta</h2>
                                <h5>Pacakage Delivery Application / Website</h5>
                                <p>Welcome to Bring Posta, the ultimate delivery package system that revolutionizes the way packages are transported and delivered. At Bugagency, we are proud to have partnered with Bring Posta to bring this innovative solution to life.</p>
                                <p>Bring Posta is a game-changer in the delivery industry, offering a seamless and efficient platform for package transportation. Our team at Bugagency has dedicated their expertise to develop and design a state-of-the-art system that optimizes the entire delivery process.</p>
                            </div>
                        </div>

                        <div class="project-item ">
                            <a class="project-item-thumbnail" href="#" target="_blank">
                                <img src="wp-content/uploads/assets/tiptop.jpg" alt="">
                            </a>
                            <div>
                                <h2>TIP TOP</h2>
                                <h5>Webshop</h5>
                                <p>At Bugagency, we are thrilled to have collaborated with Tip Top to create a top-notch e-commerce shop that offers a seamless and delightful shopping experience.</p>
                                <p>Tip Top brings together a curated selection of high-quality products from various categories, ensuring that every customer finds something to suit their taste and needs. Our team at Bugagency has worked tirelessly to develop an intuitive and visually appealing platform that showcases Tip Top's extensive product range in an easy-to-navigate manner.</p>
                            </div>
                        </div>

                        <div class="project-item reverse">
                            <a class="project-item-thumbnail" href="https://kikastoren.ch/" target="_blank">
                                <img src="wp-content/uploads/assets/kikastoren.jpg" alt="">
                            </a>
                            <div>
                                <h2>Kikastoren</h2>
                                <h5>Website</h5>
                                <p>Experience the excellence of Kikastoren, tailored specifically for the discerning clientele in Switzerland. Unleash the true potential of your WordPress website with Bugagency's unrivaled expertise and  comprehensive offerings. Together, we'll take your online presence to new heights.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_process" class="section-item section-process">
            <div class="section-item-content">
                <div class="section-item-inner">
                    <div class="container">
                        <div class="row">
                            <div class="col-md-7">
                                <div class="process-intro-layout">
                                    <div class="process-intro-layout-text">
                                        <h6>Our Process</h6>
                                        <h3 class="text-uppercase"><span class="stroked-text">Process</span> flow</h3>
                                    </div>
                                    <img src="wp-content/uploads/assets/processes.png" alt="">
                                </div>
                            </div>
                            <div class="col-md-5">
                                <div class="process_boxes_layout">
                                    <div class="process_box">
                                        <h3>01</h3>
                                        <div>
                                            <h4>Talking</h4>
                                            <p>
                                                What do you need and how we can collaborate to get the best results.
                                            </p>
                                        </div>
                                    </div>
                                    <div class="process_box">
                                        <h3>02</h3>
                                        <div>
                                            <h4>Analysing</h4>
                                            <p>
                                                Whether it is web development or branding, we stand ready to help your company grow
                                            </p>
                                        </div>
                                    </div>
                                    <div class="process_box">
                                        <h3>03</h3>
                                        <div>
                                            <h4>Working</h4>
                                            <p>
                                                As we work on the project, we share the progress with you and ask for your feedback.
                                            </p>
                                        </div>
                                    </div>
                                    <div class="process_box">
                                        <h3>04</h3>
                                        <div>
                                            <h4>Deploying</h4>
                                            <p>
                                                The end of the project is not the end of our business, we stay in touch to see how we can upgrade and update the project.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_team" class="section-item section-team">
            <div class="section-item-content">
                <div class="section-item-inner">
                    <div class="container">
                        <div class="row">
                            <div class="col-xl-8">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="team_section_intro">
                                            <h6>Our Team</h6>
                                            <h3 class="text-uppercase">Who <span class="stroked-text">We Are</span> </h3>
                                            <p>
                                                A team of dedicated developers and designers committed to providing creative solutions. Bringing ideas and innovation together to meet your demands.
                                            </p>
                                        </div>
                                    </div>
                                </div>

                                <div class="team_slider_wrapper">
                                    <div class="team_slider_arrows">
                                        <div class="team_slider_arrow prev_slide_team">PREV</div>
                                        <div class="team_slider_arrow next_slide_team">NEXT</div>
                                    </div>
                                    <div class="team_users_slider">
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/genc.jpg" alt="">
                                                <div>
                                                    <h4>Genc Maloku</h4>
                                                    <h5>Sales Departament</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>
                                                </div>
                                                <div>
                                                    <p>
                                                        In charge of enabling a smooth transition of your demand to the final product. Currently, I'm a master's student at the University of Zurich, Banking and Finance.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/gentiankrasniqi.jpg" alt="">
                                                <div>
                                                    <h4>Gentian Krasniqi</h4>
                                                    <h5>Web Developer</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>

                                                </div>
                                                <div>
                                                    <p>
                                                        Helping you to reach your desired project from start to finish. Passion is what thrives me into developing extra features for our clients.
                                                    </p>
                                                    <br>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/drilon.jpg" alt="">
                                                <div>
                                                    <h4>Drilon Maloku</h4>
                                                    <h5>Fullstack Developer</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>

                                                </div>
                                                <div>
                                                    <p>
                                                        Going an extra mile with us is the easiest so come and allow us to have the best experience :)
                                                    </p>
                                                    <br>
                                                    <br>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/asdrenxhafa.jpg" alt="">
                                                <div>
                                                    <h4>Asdren Xhafa</h4>
                                                    <h5>Fullstack Developer</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>

                                                </div>
                                                <div>
                                                    <p>
                                                        Making complicated things look simple, this is what I do :)
                                                    </p>
                                                    <br>
                                                    <br>
                                                    <br>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/xhevdet.jpg" alt="">
                                                <div>
                                                    <h4>Xhevdet Gerxhaliu</h4>
                                                    <h5>Web Developer</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>

                                                </div>
                                                <div>
                                                    <p>
                                                        Putting all the effort together to help our clients have the best experience
                                                    </p>
                                                    <br>
                                                    <br>
                                                    <br>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="team_user_box">
                                            <div class="team_user_box_heading">
                                                <img src="wp-content/uploads/assets/rinormonkey.png" alt="">
                                                <div>
                                                    <h4>Rinor Gerxhaliu</h4>
                                                    <h5>Web Developer</h5>
                                                </div>
                                            </div>
                                            <div class="team_user_box_content">
                                                <div class="team_user_box_social_media">
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.417" height="18.415" viewBox="0 0 18.417 18.415">
                                                            <g id="Group_225" data-name="Group 225" transform="translate(0)">
                                                                <path id="Path_231" data-name="Path 231" d="M1478,9651.691v-7.131h.463c.573,0,1.146,0,1.719,0,.152,0,.2-.044.217-.2.1-.81.209-1.619.314-2.43a1.47,1.47,0,0,0,0-.163h-2.723c0-.6,0-1.178,0-1.756a2.723,2.723,0,0,1,.046-.5.882.882,0,0,1,.85-.812c.373-.043.751-.035,1.128-.045.257-.007.515,0,.792,0,.006-.087.015-.153.015-.221,0-.684-.006-1.368.005-2.053,0-.186-.057-.243-.234-.244-.843-.008-1.692-.09-2.529-.023a3.122,3.122,0,0,0-2.907,2.956c-.061.808-.035,1.619-.048,2.429,0,.082,0,.164,0,.267h-2.375v2.785h2.36v7.123c-.064,0-.125.011-.185.011-2.054,0-4.107.008-6.161,0a3.437,3.437,0,0,1-3.379-2.688,3.946,3.946,0,0,1-.079-.832c0-3.791.016-7.584-.011-11.376a3.491,3.491,0,0,1,2.937-3.471,5.029,5.029,0,0,1,.606-.034c3.779,0,7.558.018,11.337-.009a3.486,3.486,0,0,1,3.433,2.626,4.422,4.422,0,0,1,.1.871q.01,5.71,0,11.419a3.428,3.428,0,0,1-2.826,3.434,6.1,6.1,0,0,1-1,.061C1479.252,9651.7,1478.638,9651.691,1478,9651.691Z" transform="translate(-1465.278 -9633.278)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.479" height="18.478" viewBox="0 0 18.479 18.478">
                                                            <g id="Group_224" data-name="Group 224" transform="translate(0)">
                                                                <path id="Path_596" data-name="Path 596" d="M2491.992,3713.479h9.636c.113.013.225.026.338.041a4.65,4.65,0,0,1,3.955,3.5c.066.259.1.528.146.792v9.889a9.645,9.645,0,0,1-.229,1.035,4.616,4.616,0,0,1-4.45,3.216c-3.038.011-6.075.005-9.113,0a4.516,4.516,0,0,1-2.708-.848,4.575,4.575,0,0,1-1.978-3.906q0-4.439,0-8.878a7.1,7.1,0,0,1,.068-.952,4.617,4.617,0,0,1,3.325-3.706C2491.315,3713.58,2491.656,3713.539,2491.992,3713.479Zm-.888,9.227c0,1.155-.005,2.31,0,3.464a2.137,2.137,0,0,0,2.217,2.257q3.5.014,7,0a2.128,2.128,0,0,0,2.21-2.224q.007-3.482,0-6.964a2.129,2.129,0,0,0-2.2-2.229c-2.339-.01-4.678,0-7.018,0a2.164,2.164,0,0,0-2.207,2.272Q2491.1,3720.992,2491.1,3722.706Z" transform="translate(-2487.588 -3713.479)" fill="#fff"/>
                                                                <path id="Path_597" data-name="Path 597" d="M2594.274,3889.564h1.747v.2q0,2.174,0,4.347a1.205,1.205,0,0,1-1.3,1.305h-6.782a1.2,1.2,0,0,1-1.3-1.289q0-2.183,0-4.365v-.192h1.73a3.1,3.1,0,0,0,1.454,3.908,2.9,2.9,0,0,0,3.429-.278A3.088,3.088,0,0,0,2594.274,3889.564Z" transform="translate(-2582.097 -3881.484)" fill="#fff"/>
                                                                <path id="Path_598" data-name="Path 598" d="M2644.8,3872.561a2.021,2.021,0,1,1,1.988,2.027A2.027,2.027,0,0,1,2644.8,3872.561Z" transform="translate(-2637.589 -3863.329)" fill="#fff"/>
                                                                <path id="Path_599" data-name="Path 599" d="M2744.582,3818.55v1.924h-1.918v-1.924Z" transform="translate(-2730.959 -3813.729)" fill="#fff"/>
                                                            </g>
                                                        </svg>
                                                    </a>
                                                    <a href="">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="18.18" height="18.291" viewBox="0 0 18.18 18.291">
                                                            <g id="Group_226" data-name="Group 226" transform="translate(0)">
                                                                <path id="Path_235" data-name="Path 235" d="M1656.276,9642.425v7.749a1.327,1.327,0,0,1-1.4,1.395h-15.393a1.332,1.332,0,0,1-1.359-1.048,1.637,1.637,0,0,1-.03-.343q0-7.755,0-15.51a1.321,1.321,0,0,1,1.385-1.391h15.406a1.32,1.32,0,0,1,1.388,1.387Zm-2.775,6.157c0-.049.006-.092.006-.135,0-1.548.007-3.1,0-4.646a6.536,6.536,0,0,0-.111-1.122,3.12,3.12,0,0,0-.942-1.793,2.921,2.921,0,0,0-1.891-.748,2.742,2.742,0,0,0-2.568,1.184c-.03.042-.06.084-.09.125l-.029,0v-1.1h-2.738v8.242h2.739v-.154c0-.509,0-1.02,0-1.529,0-1.043,0-2.086.008-3.129a1.456,1.456,0,0,1,.643-1.2,1.33,1.33,0,0,1,2.126.729,3.576,3.576,0,0,1,.107.88c.011,1.415,0,2.831,0,4.246v.156Zm-9.905-8.243h-2.727v8.243h2.727Zm-1.317-1.138c.074-.007.194-.015.312-.034a1.412,1.412,0,0,0,.155-2.758,1.957,1.957,0,0,0-.818-.037,1.413,1.413,0,0,0-1.045,2.108A1.484,1.484,0,0,0,1642.278,9639.2Z" transform="translate(-1638.095 -9633.277)" fill="#fff"/>
                                                            </g>
                                                        </svg>

                                                    </a>

                                                </div>
                                                <div>
                                                    <p>
                                                        With his energy he wants to make things better and leave no room for error!
                                                    </p>
                                                    <br>
                                                    <br>

                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                            </div>
                            <div class="col-xl-4 team_image">
                                <div class="d-flex justify-content-center align-items-center h-100">
                                    <img src="wp-content/uploads/assets/teamimage.png" alt="">
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="section_contact" class="section-item section-contact">
            <div class="section-item-content "style="background-image: url('wp-content/uploads/assets/contactbackground.png')">
                <div class="container">
                    <div class="row">
                        <div class="col-xl-7 col-md-5">
                            <div class="contact-intro">
                                <div class="row">
                                    <div class="col-md-6">
                                        <h6>Need Help?</h6>
                                        <h3><span class="stroked-text">Contact</span> Us</h3>
                                        <div class="contact-intro-content">
                                            <p>
                                                Feel free to get in touch with us to learn more about how we can help you.
                                                Get to us through our email, phone number or the contact form.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                <div class="contact-intro-info">
                                    <div class="info-box">
                                        <div class="icon">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16.645" height="12.496" viewBox="0 0 16.645 12.496">
                                                <g id="Group_163" data-name="Group 163" transform="translate(-1520.402 -782.468)">
                                                    <path id="Path_585" data-name="Path 585" d="M1537.047,863.321v7.654c-.006.009-.016.017-.018.026a1.541,1.541,0,0,1-1.666,1.3h-13.278c-.054,0-.108,0-.163,0a1.533,1.533,0,0,1-1.518-1.563q0-3.648,0-7.3a1.239,1.239,0,0,1,.013-.124c.029.016.044.023.057.032l6.743,4.505a2.5,2.5,0,0,0,3.016,0l6.027-4.024C1536.521,863.659,1536.785,863.491,1537.047,863.321Z" transform="translate(0 -77.341)" fill="#56256b"/>
                                                    <path id="Path_586" data-name="Path 586" d="M1537.067,784.221a2.211,2.211,0,0,1-.193.494,1.952,1.952,0,0,1-.48.517q-3.389,2.284-6.795,4.544a1.451,1.451,0,0,1-1.71,0q-3.412-2.266-6.812-4.548a1.451,1.451,0,0,1-.6-1.626,1.533,1.533,0,0,1,1.413-1.128c.049,0,.1,0,.146,0h13.409a1.542,1.542,0,0,1,1.538,1.051c.031.091.054.185.081.278Z" transform="translate(-0.02 0)" fill="#56256b"/>
                                                </g>
                                            </svg>
                                        </div>
                                        <a href="mailto:info@bugagency.tech">info@bugagency.tech</a>
                                    </div>
                                    <div class="info-box">
                                        <div class="icon">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="12.351" height="17.057" viewBox="0 0 12.351 17.057">
                                                <g id="Group_162" data-name="Group 162" transform="translate(-1324.049 -1351.773)">
                                                    <path id="Path_584" data-name="Path 584" d="M1330.094,1368.831a3.294,3.294,0,0,1-.426-.356,34.707,34.707,0,0,1-3.823-5.3,15.19,15.19,0,0,1-1.534-3.476,6.012,6.012,0,0,1,.794-5.188,6.169,6.169,0,0,1,9.029-1.338,6.036,6.036,0,0,1,2.2,3.857,6.849,6.849,0,0,1-.46,3.507,20.7,20.7,0,0,1-2.264,4.249,37.13,37.13,0,0,1-2.821,3.688,3.368,3.368,0,0,1-.426.356Zm.114-7.772a3.11,3.11,0,1,0-3.085-3.142A3.112,3.112,0,0,0,1330.208,1361.058Z" fill="#56256b"/>
                                                </g>
                                            </svg>
                                        </div>
                                        <p>Prishtina, Kosovo</p>
                                    </div>
                                    <div class="info-box">
                                        <div class="icon">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16.645" height="16.704" viewBox="0 0 16.645 16.704">
                                                <g id="Group_161" data-name="Group 161" transform="translate(-1530.112 -1053.044)">
                                                    <path id="Path_583" data-name="Path 583" d="M1542.591,1069.748a8.3,8.3,0,0,1-2.182-.51,15.329,15.329,0,0,1-4.4-2.608,18.9,18.9,0,0,1-4.586-5.419,10.993,10.993,0,0,1-1.283-3.487,1.4,1.4,0,0,1,.173-1.046,13.716,13.716,0,0,1,2.753-3.268c.071-.059.15-.109.225-.162a.922.922,0,0,1,1.249.093.987.987,0,0,1,.2.215,22.608,22.608,0,0,1,1.936,3.148,6.2,6.2,0,0,1,.293.76.822.822,0,0,1-.182.859c-.269.285-.565.547-.86.806-.382.336-.392.332-.19.806a7.6,7.6,0,0,0,4.372,4.245.3.3,0,0,0,.423-.1c.241-.31.509-.6.774-.892a1,1,0,0,1,1.242-.281,18.782,18.782,0,0,1,1.727.879c.666.4,1.3.845,1.942,1.278a1.242,1.242,0,0,1,.512.761.623.623,0,0,1-.058.487,3.733,3.733,0,0,1-.377.56,13.158,13.158,0,0,1-3.3,2.71A2.619,2.619,0,0,1,1542.591,1069.748Z" transform="translate(0)" fill="#56256b"/>
                                                </g>
                                            </svg>
                                        </div>
                                        <p>+383 44 652992</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-5 col-md-7">
                            <div class="contact-form">
                                <?php echo do_shortcode('[contact-form-7 id="7" title="Contact form 1"]')?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>

<?php
get_footer();
